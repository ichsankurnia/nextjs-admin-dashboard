        ...
        <>
            <div className={`z-20`}>
                ...
                <div id="sidebar" ref={sidebar}
                    ...
                    ${sidebarOpen? 'translate-x-0' : '-translate-x-64'} ${collapse? 'w-64':'md:w-[4.25rem]'} `}
                >
                    <div className={`overflow-y-auto overflow-x-hidden h-full flex flex-col items-center text-white`}>
                        <span className='mt-6 cursor-pointer z-10' onClick={() => window.location.reload()}>{collapse?'App Logo':'Logo'}</span>

                        {collapse?
                        <div className='w-full h-full px-4 mt-8 flex flex-col space-y-3 text-sm z-10'>
                        ...
                        :
                        <div className='w-full h-full mt-8 flex flex-col items-center z-10'>
                            {RouteAdminRole.map(({url_var, name_var, icon_var}, key) => 
                                <Link href={'/dashboard' + url_var} key={key}>
                        ...
                        <button onClick={()=>setCollapse(!collapse)}
                            className={`hidden md:flex absolute ${collapse? 'left-[14.6rem]':'left-[2.8rem]'} bottom-10 justify-center items-center bg-white 
